...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const startAction = await resolveStartAction(product);
  ...
  return (
    <div data-competing-sticky-action>
      ...
      <section className="section section--tight" data-sticky-final-sentinel><div className="site-container product-summary"><div><span className="eyebrow">Samengevat</span><h2 className="section-title">{product.name}</h2><ul>{product.highlights.map(highlight => <li key={highlight}><Check aria-hidden="true" />{highlight}</li>)}</ul></div><div><ClipboardCheck aria-hidden="true" /><h3>Klaar om te starten?</h3><p>Bekijk je traject en het totaalbedrag in de checkout. Betalen gebeurt veilig via Stripe zodra dit traject beschikbaar is.</p><Link className="button button--primary" data-testid="start-product" href={startAction.href}>{startAction.label} <ArrowRight aria-hidden="true" /></Link></div></div></section>
    </div>
  );
}
